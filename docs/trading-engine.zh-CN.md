...
| 真实券商下单 | 仅预留，实盘未启用 | 已选择中信证券作为未来方向；QMT/xtquant 占位适配器永久拒单，等待权限确认 |
...
| `GET /v1/trading/brokers` | 查看 paper 与中信 QMT 占位适配器的非敏感能力状态 |
...
未来券商方向已确定为中信证券，但普通个人 A 股账户是否具备 QMT、xtquant Python API 和模拟
交易权限仍未确认。仓库只加入不连接的占位适配器，详细状态和向券商确认的问题见
[中信证券 QMT / xtquant 预留适配器](brokers/citic-qmt-readiness.zh-CN.md)。权限确认后仍要逐项完成
沙箱验收、成交/撤单/查询、断线恢复、券商对账、价格笼子、交易时段、双人审批、密钥隔离和
紧急停止演练。
