...
from trading.simulation.hypothetical_orders import validate_order_clock
...
class PaperTradingEngine:
    """Local-only simulator with no external account or execution integration."""
    ...
    def submit_order(self, intent: OrderIntent, limits: RiskLimits) -> OrderResult:
        """Apply a simulated order to local cash and positions only."""
        ...
        elif clock_error := validate_order_clock(
            intent.price_time,
            limits.max_price_age_seconds,
        ):
        ...
        if intent.side == OrderSide.BUY:
            ...
            realized = (
                intent.reference_price - position.average_cost
            ) * intent.quantity - fee
            position = position.model_copy(
                update={
    ...
    def execute_protective_exits(
        ...
        """Apply stop-loss and take-profit rules to simulated positions."""
